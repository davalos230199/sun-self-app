...
// --- Componentes Auxiliares (StateButton, StateDescription, HeroAnimationSequence) ---
...
export default function Landing() {
    ...
    const NavLinks = ({ mobile = false }) => {
        ...
        // Agregamos 'como' (El Hábito) de nuevo a la navegación
    ...
    return (
        <div className="bg-white min-h-screen text-zinc-800 relative overflow-x-hidden">
            ...
            {/* FAB ELIMINADO AQUÍ (El solcito flotante ya no está) */}

            {/* Ritual Demo Modal (Placeholder - Asegúrate de tener tu componente RitualFlow real aquí) */}
            {showDemoRitual && (
                <div className="fixed inset-0 z-50 bg-white/60 backdrop-blur-lg flex items-center justify-center p-4">
                    ...
                    <div className="w-full max-w-lg relative" style={{minHeight: '400px', background: 'white', borderRadius: '2rem', padding: '2rem', display:'flex', alignItems:'center', justifyContent:'center', border:'1px solid #eee'}}>
                         <p className="text-zinc-400 font-['Patrick_Hand']">Aquí se carga el componente {"<RitualFlow />"}</p>
                         {/* <RitualFlow mode="anon" onFinish={handleFinishDemo} />  <-- DESCOMENTA ESTO CUANDO TENGAS EL COMPONENTE */}
                    </div>
                </div>
            )}
            ...
            <header className="fixed top-0 w-full z-50 bg-white/90 backdrop-blur-md shadow-sm transition-all">
                 <nav className="max-w-7xl mx-auto flex justify-between items-center p-4">
                    ...
                    {/* Mobile Controls (Ingresar + Burger) */}
                    <div className="md:hidden flex items-center">
                        {/* Botón Ingresar Móvil (Compacto y al lado del burger) */}
                        <Link to="/login" className="bg-zinc-900 text-white font-['Patrick_Hand'] text-sm px-4 py-2 rounded-full mr-3 hover:scale-105 transition-transform shadow-sm">
                            Ingresar
                        </Link>
                        {/* Botón Burger */}
                    ...
                    <AnimatePresence>
                        {isMobileMenuOpen && (
                            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="absolute top-0 left-0 w-full h-screen bg-white flex flex-col pt-24 px-6 md:hidden shadow-xl">
                                ...
                                {/* El botón Ingresar gigante se ha eliminado de aquí */}
                            </motion.div>
                        )}
                    </AnimatePresence>
                 </nav>
            ...
            <main className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-5xl relative z-10 pt-24 md:pt-32">
                ...
                {/* SECCIÓN: CIENCIA */}
                <section id="ciencia" className="mb-32 scroll-mt-24">
                    <div className="text-center mb-16">
                ...
                {/* --- SECCIÓN RESTAURADA: EL HÁBITO (Método de 2 Pasos) --- */}
            ...
            <footer className="text-center p-12 bg-white border-t border-zinc-100 relative z-10">
                <p className="text-zinc-400 text-sm font-['Patrick_Hand']">&copy; {new Date().getFullYear()} Sun Self.</p>
            </footer>
        </div>
    );
}