...
// Recibimos 'mode' (user o anon)
export default function Step5_Goal({ onFinish, ritualData, mode = 'user' }) {
    ...
    const handleSuggestMeta = async () => {
        ...
        try {
            ...
            // --- BIFURCACIÓN CLAVE ---
            if (mode === 'anon') {
                response = await api.sugerirMetaAnonimo(dataToSend);
            } else {
                response = await api.sugerirMeta(dataToSend);
            }
        ...
        >
            <div className="w-full flex flex-col items-center">
                ...
                <div className="w-48 h-48 -mt-4">
                    <Lottie animationData={animation} loop={loop} onComplete={handleRevealComplete} />
                </div>
            </div>
            ...
            <div className="w-full flex flex-col items-center gap-4 flex-grow justify-center">
                ...
                <div className="w-full flex justify-between items-center mb-1">
                    ...
                    <button
                        ...
                        className={`flex items-center px-3 py-1 text-sm font-semibold rounded-full transition-all duration-200 ${isSuggesting ? 'bg-amber-400/80 text-white cursor-not-allowed' : 'bg-amber-400 text-white hover:bg-amber-500'}`}
                    >
                        {isSuggesting ? (
                            <span className="flex items-center"><Zap size={16} className="animate-pulse mr-1" /> Pensando...</span>
                        ) : (
                            <span className="flex items-center"><Zap size={16} className="mr-1" /> Sugerir Meta</span>
                        )}
                    </button>
                </div>
                ...
                {suggestError && <p className="text-red-500 text-sm mt-2">{suggestError}</p>}
            </div>
